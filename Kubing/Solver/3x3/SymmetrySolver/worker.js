...
onmessage = e => {
    scramble = e.data[0];
    endState = e.data[1];
    // console.log(getScrStateSymmetries(scramble, endState));
    solve(scramble, endState);
}

function getScrSymmetries(scr1, scr2, step = null) {
    let symmetries = [];
    let state1, state2;
    
    for (let ori of solutionsCenter.map(ce => ce.solution)) {
        [state1, state2] = [getState(getMovesWithoutRotations([ori, scr1].join(" ")), step), getState(scr2, step)];
...
function getScrStateSymmetries(scr1, stt2, step = null) {
    let symmetries = [];
    
    let state1, state2;
    
    for (let ori of solutionsCenter.map(ce => ce.solution)) {
        [state1, state2] = [getState(getMovesWithoutRotations([ori, scr1].join(" ")), step), getPieces(stt2)];
console.log(state1);
console.log(state2);
...
function getStateSymmetries(stt1, stt2, step = null) {
    let symmetries = [];
    
    let state1, state2;
    
    for (let ori of solutionsCenter.map(ce => ce.solution)) {
        [state1, state2] = [getPieces(stt1), getPieces(stt2)];
console.log(state1);
console.log(state2);
